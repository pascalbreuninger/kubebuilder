...
KIND_VERSION="${MINIKUBE:-"latest"}" # latest is also valid
...
if [ "${KIND_VERSION}" != "none" ]; then
    ...
    if [ "$KIND_SHA256" = "automatic" ]; then
        KIND_SHA256="$(curl -sSL "https://kind.sigs.k8s.io/dl/${KIND_VERSION}/kind-linux-${architecture}.sha256sum" | awk '{print $1}')"
    fi
